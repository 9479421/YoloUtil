...
public:
    ...
    ~YoloInferencer();
    std::vector<OutResult> infer(cv::Mat& frame, float conf_threshold, float iou_threshold);

private:
    std::vector<Ort::Value> preprocess(cv::Mat& frame);
    std::vector<Ort::Value> forward(std::vector<Ort::Value>& inputTensors);
    std::vector<OutResult> postprocess(std::vector<Ort::Value>& outputTensors, float conf_threshold, float iou_threshold);
